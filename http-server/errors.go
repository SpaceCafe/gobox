...
var (
	ErrInvalidBasePath          = errors.New("http-server base path must be absolute and not end with a slash")
	ErrInvalidPort              = errors.New("http-server port must be a number between 1 and 65535")
	ErrInvalidReadHeaderTimeout = errors.New("http-server read header timeout must be greater than 0")
