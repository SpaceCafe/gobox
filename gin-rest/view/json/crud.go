...
// All operations return a render.Render to output the response as a JSON object.
type CRUD[T any] struct{}

// Create handles the creation of a new entity and returns a JSON response.
...
// Read handles reading an entity and returns a JSON response.
func (r *CRUD[T]) Read(_ types.Resource[T], entity *T, options *types.ViewOptions) render.Render {
	return r.createResponse(&[]T{*entity}, options)
}

// List handles listing entities and returns a JSON response.
...
// Update handles updating an entity and returns a JSON response.
func (r *CRUD[T]) Update(_ types.Resource[T], _ string, entity *T, options *types.ViewOptions) render.Render {
	return r.createResponse(&[]T{*entity}, options)
}

// Delete handles deleting an entity and returns a JSON response.
...
// Mime returns the MIME type for JSON responses.
func (r *CRUD[T]) Mime() string {
	return binding.MIMEJSON
}

// createResponse creates a JSON response with pagination and entity data.
